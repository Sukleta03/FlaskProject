...
class TestApi:
    ...
    def test_students_request(self, mock_response_func, client):
        ...
        response = client.get('/api/v1/student?course_name=Math')
    ...
    def test_student_add(self, mock_response, client):
        mock_response.return_value = ['255', 'Dima', 'Sukleta', 'PE-21']
        response = client.put('/api/v1/student?student_id=255&first_name=Dima&last_name=Sukleta&group_name=PE-21')
        assert response.status_code == 200
    ...
    def test_student_delete(self, mock_response_func, client):
        ...
        response = client.delete('/api/v1/student?student_id=255')
        assert response.status_code == 200
    ...
    def test_group_request(self, mock_response_func, client):
        ...
        response = client.get('/api/v1/group?student_count=15')
        assert response.status_code == 200
    ...
    def test_add_to_course(self, mock_response, client):
        ...
        response = client.put('/api/v1/course?student_id=255&course_name=Math+English')
        assert response.status_code == 200
    ...
    def test_delete_from_course(self, mock_response, client):
        ...
        response = client.delete('/api/v1/course?student_id=255&course_name=Math')
        assert response.status_code == 200
